"use client";

import { Suspense, useMemo, useState } from "react";
import { signIn } from "next-auth/react";
import { useRouter, useSearchParams } from "next/navigation";
import Link from "next/link";
import { ArrowRight, KeyRound, LoaderCircle, ShieldCheck } from "lucide-react";

import { AppLogo } from "@/components/tracker/AppLogo";

function LoginPageContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const callbackUrl = searchParams.get("callbackUrl") ?? "/";
  const authError = searchParams.get("error");

  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [oauthLoading, setOauthLoading] = useState<"google" | "discord" | null>(null);

  const queryError = useMemo(() => {
    if (!authError) return null;
    if (authError === "AccessDenied") {
      return "Accès refusé. Vérifie que ton compte n'est pas suspendu.";
    }
    if (authError === "OAuthAccountNotLinked") {
      return "Ce fournisseur est déjà lié à un autre mode de connexion pour cet email.";
    }
    if (authError === "Configuration") {
      return "La connexion est temporairement indisponible. Réessaie dans un instant.";
    }
    return "La connexion n'a pas pu être finalisée. Réessaie.";
  }, [authError]);

  async function handleCredentials(e: React.FormEvent) {
    e.preventDefault();
    if (loading || oauthLoading) return;
    setError(null);
    setLoading(true);

    const res = await signIn("credentials", {
      email,
      password,
      redirect: false,
    });

    setLoading(false);

    if (res?.error === "banned") {
      router.push("/suspended");
    } else if (res?.error) {
      setError("Email ou mot de passe incorrect.");
    } else {
      router.push(callbackUrl);
    }
  }

  async function handleOAuth(provider: "google" | "discord") {
    if (loading || oauthLoading) return;
    setError(null);
    setOauthLoading(provider);
    await signIn(provider, { callbackUrl });
  }

  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#1a1d29_0%,#0d0e12_42%)] px-4 py-10 text-foreground">
      <div className="mx-auto flex min-h-[calc(100vh-5rem)] w-full max-w-5xl items-center justify-center">
        <div className="grid w-full max-w-4xl gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <section className="rounded-[28px] border border-gold/12 bg-[linear-gradient(160deg,rgba(23,25,32,0.96),rgba(13,14,18,0.98))] p-6 shadow-[0_22px_60px_rgba(0,0,0,0.35)] sm:p-8">
            <div className="max-w-md space-y-5">
              <AppLogo />
              <div className="space-y-3">
                <span className="inline-flex items-center gap-2 rounded-full border border-gold/18 bg-gold/[0.08] px-3 py-1 text-[11px] uppercase tracking-[0.16em] text-gold/84">
                  <ShieldCheck className="h-3.5 w-3.5" />
                  Authentification
                </span>
                <h1 className="font-heading text-3xl leading-tight text-foreground sm:text-4xl">
                  Reprends la veille de tes achats Yu-Gi-Oh!
                </h1>
                <p className="text-sm leading-6 text-muted-foreground/82 sm:text-[15px]">
                  Connecte-toi pour retrouver ta watchlist, tes vendeurs suivis,
                  ton plan d&apos;achat et les derniers ajustements de prix.
                </p>
              </div>

              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl border border-border/60 bg-card/60 px-4 py-3">
                  <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground/78">
                    Focus
                  </p>
                  <p className="mt-1 text-sm text-foreground/90">Watchlist, dashboard, plan</p>
                </div>
                <div className="rounded-2xl border border-border/60 bg-card/60 px-4 py-3">
                  <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground/78">
                    Accès
                  </p>
                  <p className="mt-1 text-sm text-foreground/90">Email, Google ou Discord</p>
                </div>
              </div>
            </div>
          </section>

          <section className="rounded-[28px] border border-border/70 bg-[linear-gradient(180deg,rgba(19,20,24,0.96),rgba(12,13,18,0.98))] p-6 shadow-[0_18px_48px_rgba(0,0,0,0.28)] sm:p-8">
            <div className="space-y-5">
              <div className="space-y-2">
                <p className="text-[11px] uppercase tracking-[0.16em] text-muted-foreground/78">
                  Connexion
                </p>
                <h2 className="text-2xl font-semibold text-foreground/94">
                  Entrer dans le tracker
                </h2>
                <p className="text-sm leading-6 text-muted-foreground/82">
                  Utilise ton compte habituel ou choisis un fournisseur OAuth.
                </p>
              </div>

              {(error || queryError) && (
                <p className="rounded-2xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">
                  {error || queryError}
                </p>
              )}

              <form onSubmit={handleCredentials} className="space-y-4">
                <div>
                  <label className="mb-1.5 block text-xs uppercase tracking-[0.12em] text-muted-foreground/78">
                    Email
                  </label>
                  <input
                    type="email"
                    required
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    disabled={loading || oauthLoading !== null}
                    className="w-full rounded-2xl border border-border bg-surface-raised px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground/70 focus:border-gold focus:outline-none transition-colors"
                    placeholder="vous@exemple.com"
                  />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs uppercase tracking-[0.12em] text-muted-foreground/78">
                    Mot de passe
                  </label>
                  <input
                    type="password"
                    required
                    value={password}
                    onChange={(e) => setPassword(e.target.value)}
                    disabled={loading || oauthLoading !== null}
                    className="w-full rounded-2xl border border-border bg-surface-raised px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground/70 focus:border-gold focus:outline-none transition-colors"
                    placeholder="••••••••"
                  />
                </div>
                <button
                  type="submit"
                  disabled={loading || oauthLoading !== null}
                  className="flex w-full items-center justify-center gap-2 rounded-2xl border border-gold/25 bg-gold px-4 py-3 text-sm font-semibold text-background transition-colors hover:bg-gold/92 disabled:opacity-50"
                >
                  {loading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <KeyRound className="h-4 w-4" />}
                  {loading ? "Connexion…" : "Se connecter"}
                </button>
              </form>

              <div className="flex items-center gap-3">
                <div className="h-px flex-1 bg-border" />
                <span className="text-xs uppercase tracking-[0.12em] text-muted-foreground/78">ou</span>
                <div className="h-px flex-1 bg-border" />
              </div>

              <div className="space-y-2">
                <button
                  onClick={() => handleOAuth("google")}
                  disabled={loading || oauthLoading !== null}
                  className="flex w-full items-center justify-center gap-2 rounded-2xl border border-border bg-surface-raised py-3 text-sm text-foreground/90 transition-colors hover:border-gold/40 hover:bg-card/75 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {oauthLoading === "google" ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <GoogleIcon />}
                  {oauthLoading === "google" ? "Redirection Google…" : "Continuer avec Google"}
                </button>
                <button
                  onClick={() => handleOAuth("discord")}
                  disabled={loading || oauthLoading !== null}
                  className="flex w-full items-center justify-center gap-2 rounded-2xl border border-border bg-surface-raised py-3 text-sm text-foreground/90 transition-colors hover:border-gold/40 hover:bg-card/75 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {oauthLoading === "discord" ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <DiscordIcon />}
                  {oauthLoading === "discord" ? "Redirection Discord…" : "Continuer avec Discord"}
                </button>
              </div>

              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border/70 pt-4">
                <div className="flex flex-col gap-2">
                  <p className="text-xs text-muted-foreground/82">
                    Pas encore de compte ?
                  </p>
                  <Link
                    href="/forgot-password"
                    className="text-xs text-muted-foreground/82 transition-colors hover:text-gold"
                  >
                    Mot de passe oublié ?
                  </Link>
                </div>
                <Link
                  href="/register"
                  className="inline-flex items-center gap-1 text-sm font-medium text-gold transition-colors hover:text-gold/80"
                >
                  S&apos;inscrire
                  <ArrowRight className="h-4 w-4" />
                </Link>
              </div>
            </div>
          </section>
        </div>
      </div>
    </div>
  );
}

export default function LoginPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-[radial-gradient(circle_at_top,#1a1d29_0%,#0d0e12_42%)] px-4 py-10 text-foreground">
          <div className="mx-auto flex min-h-[calc(100vh-5rem)] w-full max-w-5xl items-center justify-center">
            <div className="rounded-[28px] border border-border/70 bg-[linear-gradient(180deg,rgba(19,20,24,0.96),rgba(12,13,18,0.98))] px-8 py-10 text-sm text-muted-foreground/82">
              Chargement de la connexion…
            </div>
          </div>
        </div>
      }
    >
      <LoginPageContent />
    </Suspense>
  );
}

function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
    </svg>
  );
}

function DiscordIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="#5865F2">
      <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
    </svg>
  );
}
